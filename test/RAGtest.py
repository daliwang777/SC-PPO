...
#     project="RLGNN",
#     name='Qwen-7B-new-RAGtest-withonlytrain',
...
# 将每个问题插入到模板中
    ...
    for item in generated_data:
        results = loaded_db.similarity_search(item["question"], k=5)
        history=""
        for i in results:
            history=history+i.page_content+"\n"
        item["question"] = prompt_template.format(item["question"])
        item["question"]="基于历史的工作,你有以下历史经验:"+history+item["question"]

...
- 输出格式必须为<>包裹起来的函数名称。
- 你只能从已知的函数中选择一个输出，如果你确信可以通过压测的方式探测则可以重复使用函数,请体现你的思考过程.
...
class PPOTrainer:
    def __init__(self, model_path, data, gamma=0.8, clip=0.2, entropy=0.1):
        ...
        # lora_path = "/share/project/daliwang/daliwang/RL/loraqwen/policy"
        # lora_path1="/share/project/daliwang/daliwang/GCRRL/new/lora_qwen25_tr7B"
        # lora_path="/share/project/daliwang/daliwang/GCRRL/new/lora_qwq_tr5_final"
...
# 将每个问题插入到模板中
    with open("/share/project/daliwang/daliwang/GCRRL/new/alignment_final_test1_can_reward.json",'r',encoding='utf-8') as file:
        data=json.load(file)
    generated_data = []
    for i in data:
    ...
    trainer=PPOTrainer(model_path,generated_data)
    ...
    right=0
    all=0
    questmp="I noticed that transferring data between my machines keeps showing Runtime Error, how do I check my machine's environment for problems?"
    questmp=prompt_template.format(questmp)
    for i in tqdm(range(len(generated_data))):
        i=151
        #print(f"Question:{generated_data[i]['question']}")
        #question=trainer.tokenizer(generated_data[i]['question'],return_tensors='pt').to(trainer.base_model.device)
        #output,new_tokens=trainer.generate(question["input_ids"])
        question=trainer.tokenizer(questmp,return_tensors='pt').to(trainer.base_model.device)
        output,new_tokens=trainer.generate(question["input_ids"])
        reward=trainer.get_reward(output,new_tokens,index=i,flag=1)
        if reward>0:
            right+=1
        all+=1
        break
        # print(f"Correct ratio:{right/all}")
        # wandb.log({"Correct ratio":right/all,"reward":reward})
    print(f"Correct ratio:{right/all}")
    wandb.finish()
    # trainer.train()
    # wandb.finish()
